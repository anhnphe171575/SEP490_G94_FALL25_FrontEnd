...
interface TaskDetailsDevelopmentProps {
  taskId: string | null;
  projectId?: string;
}
...
export default function TaskDetailsDevelopment({ taskId, projectId }: TaskDetailsDevelopmentProps) {
  ...
  const createBranch = async () => {
    if (!taskId || !selectedRepo || !branchName.trim()) {
  ...
  const removeLink = async (linkId: string, linkType: string, isBranch: boolean = false) => {
    ...
    if (isBranch) {
      ...
      const choice = window.confirm(
        ...
        `❌ Cancel = Only unlink from task (branch stays on GitHub)\n\n` +
        `Click OK to DELETE on GitHub, Cancel to just unlink.`
      );
      
      console.log('User choice:', choice ? 'DELETE on GitHub' : 'Just unlink');
      
      // choice = true (OK) → delete on GitHub
      // choice = false (Cancel) → just unlink
      deleteOnGithub = choice;
      
    } else {
      // For commits/PRs, just confirm unlink
      const confirmMessage = `Remove this ${linkType} from the task? This will only unlink it, the ${linkType} will still exist on GitHub.`;
    ...
    try {
      ...
      const url = `/api/tasks/${taskId}/github/links/${linkId}${deleteOnGithub ? '?deleteOnGithub=true' : ''}`;
      ...
      if (deleteOnGithub && response.data.deletedOnGithub) {
        alert('✅ Branch deleted on GitHub and unlinked from task!');
      } else if (deleteOnGithub && !response.data.deletedOnGithub) {
        console.warn('⚠️ deleteOnGithub was true but deletedOnGithub is false in response');
      }
  ...
  return (
    <Box sx={{ bgcolor: '#ffffff' }}>
      ...
      {/* Connected Repositories - Jira Style (Moved to bottom) */}
        ...
        <Stack direction="row" spacing={1} alignItems="flex-start">
          ...
          <Box>
            ...
            <Typography fontSize="11px" color="#9a6700">
              Include the task ID in your commit message or PR title/description, and they'll be automatically linked!
              <br />
              Example: <code style={{ padding: '2px 4px', background: '#fff', borderRadius: 3 }}>
                git commit -m "Fix login bug (Task: {taskId})"
              </code>
            </Typography>
          </Box>
        </Stack>
          ...
          <Stack spacing={2.5} sx={{ pt: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                setError(null); // Clear error when user changes token
                setTokenVerified(false); // Reset verification
              }}
